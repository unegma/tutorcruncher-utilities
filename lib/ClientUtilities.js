...
class ClientUtilities extends BaseUtilities {
  ...
  /**
   * Get all the Clients in the system with full details
   * WARNING, THIS WILL CREATE A LOT OF API REQUESTS
   *
   * @param delayIncrement - this will prevent all get requests firing at once
   * @param clientIdsArray
   * @returns {Promise<unknown[]>}
   */
  getFullClients = async (clientIdsArray, delayIncrement = 300) => {
    console.log('Getting Full Clients');
    return await this.getFullItems(clientIdsArray, delayIncrement, this.getClient.bind(this));
  }
